	...
	<div class="row">
		<div class="col-md-10 col-md-offset-1">
			...
			<div class="panel panel-default">
				<div class="panel-heading">Sent Items <?php $gr = (Session::get('group')=='Off') ? '1' : '0' ; ?>
				...
				<div class="panel-body">
					...
					<?php $i = ($data->currentPage() - 1) * $data->perPage() + 1; ?>
						...
						<tr>
							...
							<td>{{$i}}</td>
							<td><a href="@if(!$gr){{url('inbox/group').'?p='.urlencode($value->SenderNumber)}}@else{{url('inbox/'.$value->ID)}}@endif">{{$value->SenderNumber}}</a></td>
							<td>{{str_limit($value->TextDecoded, 20)}}</td>
							<td>{{$value->ReceivingDateTime}}</td>
						</tr>
						<?php $i++; ?>
						@endforeach
					</table>
