...
<script type="text/javascript">
	...
	function getData (term,page) {
		...
		$.get("{{url('twitter')}}?page="+page+"&term="+term, function(data,status){
			var res= '';
	...
	{
	    $(document).bind("ajaxStart.mine2",function(){
		...
		$.get("{{url('twitter/timeline')}}", function(data,status){
		    ...
		    {
			    console.log(item);
		    	res += '<span id="l-'+item.id+'" href="#" class="list-group-item"><p class="list-group-item-heading"><b>'+item.user['name']+'</b> (@'+item.user['screen_name']+') '+item.created_at+'</p><p class="list-group-item-text">'+item.text+'<a title="Teruskan sebagai sms" href=\"{{url("inbox")}}?text='+encodeURIComponent(item.text)+'\"><span class="pull-right"><span style="color:blue" class="glyphicon glyphicon-arrow-right"></a></span></span></p></span>';   
			})
			$(".member").html(res);
		});
		$(document).unbind(".mine2");
	}
